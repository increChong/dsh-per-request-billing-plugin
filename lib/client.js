...
const T = {
  ...
  unavailable: '未能读取该提供方的模型清单，请刷新页面。',
  loading: '读取模型配置…',
  inherited: '跟随提供方',
  explicit: '单独设置',
  marked: '按次计费',
...
function plugin(store, write, recheck) {
  ...
  function ProviderCard(props) {
    ...
    if (provider === undefined) {
      // A provider can be added while this page is open, and the catalog read
      // at load time cannot know about it. Ask the host again rather than
      // dead-ending the card; `recheck` ignores a route it already re-asked
      // for, so this settles on the message instead of looping.
      if (recheck(route)) return hint(T.loading)
      return hint(state.error ?? T.unavailable, state.error !== null)
    }
...
exports.apply = function apply(ctx) {
  ...
  /**
   * Re-read the catalog once for a provider a card could not find, which is
   * what a provider added after this page loaded looks like. Each route is
   * re-asked at most once, so a route the host genuinely does not report
   * settles on the message instead of polling.
   * @param route - the provider route the card is rendering.
   * @returns whether a refresh was started.
   */
  const rechecked = new Set()
  const recheck = (route) => {
    if (route.length === 0 || rechecked.has(route)) return false
    rechecked.add(route)
    refresh()
    return true
  }

  const ui = plugin(store, write, recheck)
