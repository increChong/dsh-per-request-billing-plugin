...
/**
 * Collect every route candidate that could describe the request being
 * assembled, then let the caller inject when ANY of them is marked.
 *
 * `agent.options` is written when the session starts and is NOT updated when
 * the model is switched mid-session — trusting it alone silently disables the
 * optimization. So every plausible source is gathered:
 *
 *   1. `agentDefaultModel.currentSelection()` — the selection owner
 *   2. the session `modelSelection` projection — `pending`, then `lastUsed`
 *   3. `agent.options` — the session-start default
 *   4. the session's logged request header — the last route actually used
 *   5. `assembly.variables.provider/model` — resolved in this waterfall
...
function resolveCandidates(context, assembly, agent) {
  ...
  if (agent !== null && agent !== undefined) {
    const defaults = context.get('agentDefaultModel')
    if (defaults !== undefined) {
      try {
        const selection = defaults.currentSelection()
        if (selection !== null && selection !== undefined) push(selection.provider, selection.model, 'agent-default-model')
      } catch {
        // Service absent or not ready: skip this candidate.
      }
    }
    ...
    if (projections !== undefined) {
      try {
        ...
        if (projected !== null && projected !== undefined) {
          const { pending, lastUsed } = projected
          if (pending !== null && pending !== undefined) push(pending.provider, pending.model, 'session-pending')
          if (lastUsed !== null && lastUsed !== undefined) push(lastUsed.provider, lastUsed.model, 'session-last-used')
        }
      } catch {
...
export function apply(ctx) {
  ...
  /**
   * Persist one mark. Writing through the settings scope is what makes the
   * choice durable; `update` merges, so unrelated marks are never restated.
  ...
  async function write(body) {
    ...
    if (scopeKind === 'global') {
      await scope.update({ global: body.on === true })
      return
    }
    if (scopeKind === 'provider') {
      const provider = String(body.provider)
      const next = { ...config.providerDefault }
      if (body.on === true) next[provider] = 'per-request'
      else delete next[provider]
      await scope.update({ providerDefault: next })
      return
    }
    if (scopeKind === 'model') {
      const key = String(body.provider) + '/' + String(body.model)
      const next = { ...config.models }
      if (body.on === true) next[key] = 'per-request'
      else delete next[key]
      await scope.update({ models: next })
      return
    }
  ...
  ctx.on('system-prompt/assemble', async (assembly, context, next) => {
    ...
    try {
      ...
      const candidates = resolveCandidates(ctx, assembled, agent)
      const hit = candidates.find((candidate) => modelMode(config, candidate.provider, candidate.model) === 'per-request')
      if (hit === undefined) return assembled
      ...
      const injected = { name: SECTION, text: sectionText(hit.provider, hit.model, toolNames) }
  ...
  ctx.tools.register(defineTool({
    name: 'billing_set',
    description: '切换「按次计费」标记。scope=provider 时按整个提供方切换（其下所有模型继承）；scope=model 时必须同时给出 provider 与 model，只切换该模型；scope=global 时切换全局开关。修改会持久化，从下一个模型步骤开始生效。',
    parameters: {
      ...
      on: { type: 'boolean', required: true, description: 'true=按次计费，false=取消按次计费标记。' },
    },
    output: { schema: { type: 'json' }, render: textRender },
    async execute(args) {
    ...
    ctx.effect(() => webServer.register({
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          send(400, { ok: false, message: error instanceof Error ? error.message : String(error) })
        }
      },
